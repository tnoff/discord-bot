...
class RoleAssignment(CogHelper):
    ...
    def __init__(self, bot, logger, settings, db_engine):
        ...
        '''
        super().__init__(bot, logger, settings, db_engine, settings_prefix='role', section_schema=ROLE_SECTION_SCHEMA)
        if not settings.get('include', {}).get('role', False):
            raise CogMissingRequiredArg('Role cog not enabled')
