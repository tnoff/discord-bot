...
The music system operates through **six independent background loops** that run continuously throughout the bot's lifecycle. Each loop handles a specific responsibility, working together to provide seamless music playback, caching, and user interaction.
...
**Iteration Rate**: ~10-100ms (fastest loop)
...
**Iteration Rate**: ~10ms between attempts (actual downloads take 30s+ due to backoff)
...
**Iteration Rate**: ~10ms between searches (actual API calls vary)
...
- Downloads are slow (~30s+ with backoff)
...
**Iteration Rate**: 1 second (slowest loop, infrequent checks needed)
...
**Iteration Rate**: ~10ms between attempts (processes queued items as they arrive)
...
| Loop | Iteration Rate | Blocking Operations | Priority |
|------|---------------|---------------------|----------|
| Send Messages | 10-100ms | Discord API calls | Highest |
| Download Files | 10ms | yt-dlp downloads (30s+) | High |
| YouTube Search | 10ms | YT Music API (~500ms) | High |
| Cleanup Players | 1000ms | Player cleanup | Low |
| Cache Cleanup | 1000ms | File deletion, S3 upload | Low |
| History Update | 10ms | Database writes | Medium |

**Why Different Rates?**:
- Fast loops (10ms): Process frequent events (downloads, messages)
- Slow loops (1s): Infrequent maintenance tasks (cleanup, monitoring)

---

## Key Design Principles

1. **Separation of Concerns**: Each loop has one primary responsibility
2. **Independent Operation**: Loops don't directly depend on each other
3. **Queue-Based Communication**: Loops communicate via queues, not direct calls
4. **Graceful Degradation**: Failures in one loop don't affect others
5. **Fair Resource Distribution**: Distributed queues prevent guild monopolization
6. **Idempotent Operations**: Safe to retry on failure
7. **Shutdown Safety**: All loops exit cleanly when shutdown is requested

This architecture enables concurrent processing of multiple guilds' music requests while maintaining fair resource distribution and robust error handling.
