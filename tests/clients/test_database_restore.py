...
from tests.helpers import fake_async_file_engine as db_engine  #pylint:disable=unused-import


@pytest_asyncio.fixture
async def backup_client(db_engine):  #pylint:disable=redefined-outer-name
...
async def test_restore_with_clear_existing(backup_client, db_engine):  #pylint:disable=redefined-outer-name
    ...
    backup_data = {'guild': [{'id': 1, 'server_id': 123456789}]}
    with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as f:
        json.dump(backup_data, f)
        backup_file = Path(f.name)
...
async def test_restore_nonexistent_file(backup_client):  #pylint:disable=redefined-outer-name
    ...
    with pytest.raises(FileNotFoundError):
        await backup_client.restore_backup(Path('/nonexistent/path/backup.json'))
