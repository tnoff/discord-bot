...
@pytest.mark.asyncio()
async def test_download_queue(mocker, fake_context, fake_stores):  #pylint:disable=redefined-outer-name
...
async def test_download_queue_download_error(mocker, fake_context):  #pylint:disable=redefined-outer-name
    ...
    cog = Music(fake_context['bot'], BASE_MUSIC_CONFIG, fake_context['dispatcher'])
    attach_in_process_broker(cog)
    attach_in_process_download(cog, worker_cls=yield_download_worker_download_error())
    s = fake_source_dict(fake_context)
    await cog.get_player(fake_context['guild'].id, ctx=fake_context['context'])
    await cog.download_client.submit(fake_context['guild'].id, s)
    await cog.download_client.run(cog.bot_shutdown_event)
...
async def test_download_queue_no_result(mocker, fake_context):  #pylint:disable=redefined-outer-name
    ...
    cog = Music(fake_context['bot'], BASE_MUSIC_CONFIG, fake_context['dispatcher'])
    attach_in_process_broker(cog)
    attach_in_process_download(cog, worker_cls=yield_fake_download_worker(None))
    cog.dispatcher = MagicMock()
    await cog.get_player(fake_context['guild'].id, ctx=fake_context['context'])
    await cog.download_client.submit(fake_context['guild'].id, s)
    await cog.download_client.run(cog.bot_shutdown_event)
...
async def test_download_retry_limit_exceeded(mocker, fake_context):  # pylint: disable=redefined-outer-name
    ...
    cog = Music(fake_context['bot'], BASE_MUSIC_CONFIG, fake_context['dispatcher'])
    attach_in_process_broker(cog)
    attach_in_process_download(cog, worker_cls=yield_download_worker_retry_limit_exceeded())
    cog.dispatcher = MagicMock()
    s = fake_source_dict(fake_context)
    await cog.get_player(fake_context['guild'].id, ctx=fake_context['context'])
    await cog.download_client.submit(fake_context['guild'].id, s)
...
async def test_download_playlist_add_request_no_ytdlp_data(mocker, fake_context, fake_stores):  # pylint: disable=redefined-outer-name
    ...
    await cog.broker_client.register_request(req)
    await cog.download_client.submit(fake_context['guild'].id, req)
    await cog.download_client.run(cog.bot_shutdown_event)
    await cog.process_download_results()
    from discord_core.cogs.music_helpers.common import MediaRequestLifecycleStage  # pylint: disable=import-outside-toplevel
    assert req.lifecycle_stage == MediaRequestLifecycleStage.FAILED


@pytest.mark.asyncio()
async def test_download_playlist_add_request_cache_hit(mocker, fake_context, fake_stores):  # pylint: disable=redefined-outer-name
...
@pytest.mark.asyncio()
async def test_download_files_runs_cache_cleanup(mocker, fake_context, fake_stores):  #pylint:disable=redefined-outer-name
    ...
    with TemporaryDirectory() as tmp_dir:
        with fake_media_download(tmp_dir, fake_context=fake_context) as sd:
            ...
            cog.broker_client.cache_cleanup = AsyncMock(return_value=True)

            await cog.download_client.submit(fake_context['guild'].id, sd.media_request)
            await cog.download_client.run(cog.bot_shutdown_event)
            await cog.process_download_results()

            cog.broker_client.cache_cleanup.assert_awaited_once()
...
async def test_ensure_video_download_result_none_pushes_failed(mocker, fake_context):  # pylint: disable=redefined-outer-name
    ...
    await cog.broker_client.register_request(media_request)
...
async def test_download_video_too_long_marks_request_rejected(mocker, fake_context):  # pylint: disable=redefined-outer-name
    ...
    cog = Music(fake_context['bot'], BASE_MUSIC_CONFIG, fake_context['dispatcher'])
    attach_in_process_broker(cog)
    attach_in_process_download(cog, worker_cls=yield_download_worker_video_too_long())
    cog.dispatcher = MagicMock()
    s = fake_source_dict(fake_context)
    await cog.get_player(fake_context['guild'].id, ctx=fake_context['context'])
    await cog.broker_client.register_request(s)
    await cog.download_client.submit(fake_context['guild'].id, s)
    await cog.download_client.run(cog.bot_shutdown_event)
