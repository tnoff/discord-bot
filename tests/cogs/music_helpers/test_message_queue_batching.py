...
def test_update_batch_item():
    ...
    result = queue.update_batch_item(batch_id, source_uuid, MessageStatus.DOWNLOADING)
    ...
    assert batch.status_map[source_uuid] == MessageStatus.DOWNLOADING
...
def test_update_batch_item_with_error():
    ...
    result = queue.update_batch_item(batch_id, source_uuid, MessageStatus.FAILED, error_msg)

    assert result is True

    # Check error was stored
    batch = queue.active_batches[batch_id]
    assert batch.status_map[source_uuid] == MessageStatus.FAILED
...
def test_cleanup_completed_batch():
    ...
    for source_dict in source_dicts:
        queue.update_batch_item(batch_id, str(source_dict.uuid), MessageStatus.COMPLETED)
...
def test_get_batch_update():
    ...
    queue.update_batch_item(batch_id, str(source_dicts[0].uuid), MessageStatus.DOWNLOADING)
