...
async def test_spotify_message_check():
    ...
    with pytest.raises(InvalidSearchURL) as exc:
        await x.check_source('https://open.spotify.com/playlist/1111', '1234', 'foo bar requester', '2345', None)
...
async def test_spotify_throw_exception():
    ...
    with pytest.raises(ThirdPartyException) as exc:
        await x.check_source('https://open.spotify.com/album/1111', '1234', 'foo bar requester', '2345', loop)
...
async def test_spotify_album_get():
    ...
    result = await x.check_source('https://open.spotify.com/album/1111', '1234', 'foo bar requester', '2345', loop)
...
async def test_spotify_album_get_shuffle():
    loop = asyncio.get_running_loop()
    x = DownloadClient(None, spotify_client=MockSpotifyClient())
    result = await x.check_source('https://open.spotify.com/album/1111 shuffle', '1234', 'foo bar requester', '2345', loop)
    assert result[0].requester_id == '2345'
    assert result[0].search_string == 'foo track foo artists'
    assert result[0].search_type == SearchType.SPOTIFY
...
async def test_spotify_playlist_get():
    loop = asyncio.get_running_loop()
    x = DownloadClient(None, spotify_client=MockSpotifyClient())
    result = await x.check_source('https://open.spotify.com/playlist/1111', '1234', 'foo bar requester', '2345', loop)
    assert result[0].search_string == 'foo track foo artists'
...
async def test_spotify_playlist_get_shuffle():
    loop = asyncio.get_running_loop()
    x = DownloadClient(None, spotify_client=MockSpotifyClient())
    result = await x.check_source('https://open.spotify.com/playlist/1111 shuffle', '1234', 'foo bar requester', '2345', loop)
    assert result[0].search_string == 'foo track foo artists'
...
async def test_spotify_track_get():
    loop = asyncio.get_running_loop()
    x = DownloadClient(None, spotify_client=MockSpotifyClient())
    result = await x.check_source('https://open.spotify.com/track/1111', '1234', 'foo bar requester', '2345', loop)
    assert result[0].search_string == 'foo track foo artists'
...
async def test_youtube_no_creds():
    ...
    x = DownloadClient(None)
    with pytest.raises(InvalidSearchURL) as exc:
        await x.check_source('https://www.youtube.com/playlist?list=11111', '1234', 'foo bar requester', '2345', loop)
...
async def test_youtube_playlist():
    ...
    result = await x.check_source('https://www.youtube.com/playlist?list=11111', '1234', 'foo bar requester', '2345', loop)
...
async def test_youtube_playlist_shuffle():
    loop = asyncio.get_running_loop()
    x = DownloadClient(None, youtube_client=MockYoutubeClient())
    result = await x.check_source('https://www.youtube.com/playlist?list=11111 shuffle', '1234', 'foo bar requester', '2345', loop)
    assert result[0].search_string == 'https://example.foo.com'
    assert result[0].search_type == SearchType.DIRECT
...
async def test_youtube_short():
    loop = asyncio.get_running_loop()
    x = DownloadClient(None)
    result = await x.check_source('https://www.youtube.com/shorts/aaaaaaaaaaa?extra=foo', '1234', 'foo bar requester', '2345', loop)
...
async def test_youtube_video():
    loop = asyncio.get_running_loop()
    x = DownloadClient(None)
    result = await x.check_source('https://www.youtube.com/watch?v=aaaaaaaaaaa?extra=foo', '1234', 'foo bar requester', '2345', loop)
...
async def test_fxtwitter():
    loop = asyncio.get_running_loop()
    x = DownloadClient(None)
    result = await x.check_source('https://fxtwitter.com/NicoleCahill_/status/1842208144073576615', '1234', 'foo bar requester', '2345', loop)
...
async def test_basic_search():
    loop = asyncio.get_running_loop()
    x = DownloadClient(None)
    result = await x.check_source('foo bar', '1234', 'foo bar requester', '2345', loop)
