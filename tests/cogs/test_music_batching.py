...
from discord_bot.cogs.music_helpers.batched_message import ItemStatus, BatchedMessageItem
...
async def test_update_batch_item_status(fake_engine, fake_context):  #pylint:disable=redefined-outer-name
    ...
    cog.update_batch_item_status(source_dict, ItemStatus.DOWNLOADING)

    # Should have called update_item_status on the batch
    mock_batch.update_item_status.assert_called_once_with(str(source_dict.uuid), ItemStatus.DOWNLOADING, None)

    # Should not call update if no batch_id
    source_dict_no_batch = fake_source_dict(fake_context)
    cog.update_batch_item_status(source_dict_no_batch, ItemStatus.DOWNLOADING)
...
def test_batch_message_content_generation():
    ...
    spotify_dict.search_string = "spotify:track:4iV5W9uYEdYUVa79Applp"

    youtube_dict = fake_source_dict(test_context)
    youtube_dict.search_string = "https://www.youtube.com/watch?v=dQw4w9WgXcQ"

    search_dict = fake_source_dict(test_context)
    search_dict.search_string = "Never Gonna Give You Up Rick Astley"
    ...
    batch.update_item_status(str(youtube_dict.uuid), ItemStatus.DOWNLOADING)
    batch.update_item_status(str(search_dict.uuid), ItemStatus.FAILED, "video unavailable")

    content = batch.generate_message_content()

    # Check content includes all the different types
    assert "spotify:track:4iV5W9uYEdYUVa79Applp" in content
    # URL should be wrapped in <> to prevent Discord embeds
    assert "<https://www.youtube.com/watch?v=dQw4w9WgXcQ> (downloading...)" in content
    assert "Never Gonna Give You Up Rick Astley (failed: video unavailable)" in content
...
async def test_large_playlist_scenario(fake_engine, fake_context):  #pylint:disable=redefined-outer-name
    ...
    for i in range(30):
        source_dict = fake_source_dict(fake_context)
        if i % 3 == 0:
            source_dict.search_string = f"spotify:track:track{i}"
        elif i % 3 == 1:
