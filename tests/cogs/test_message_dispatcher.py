...
@pytest.mark.asyncio
async def test_remove_mutable_lock_not_acquired_reenqueues(mocker):
    '''_remove_mutable re-enqueues when acquire_lock returns False (e.g. Redis contention).

    Regression for the duplicate "Processing" status-message bug: a remove that
    ran concurrently with an in-flight create deleted nothing and orphaned the
    just-sent message. The remove must wait on the same per-key lock instead.
    '''
    dispatcher = make_dispatcher()
    ...
    await dispatcher._remove_mutable('k')  # pylint: disable=protected-access

    # Bundle still present (nothing deleted) and the remove was re-queued.
    assert await dispatcher._bundle_store.load('k') is not None  # pylint: disable=protected-access
    q = dispatcher._work_queue._queue  # pylint: disable=protected-access
    assert not q.empty()
    _pri, _seq, member, _payload = q.get_nowait()
    assert member == 'remove:k'


@pytest.mark.asyncio
async def test_remove_mutable_releases_lock(mocker):
    '''_remove_mutable releases the per-key lock after completing.'''
    dispatcher = make_dispatcher()
    release_spy = mocker.spy(dispatcher._work_queue, 'release_lock')  # pylint: disable=protected-access
    bundle = MessageMutableBundle(guild_id=1, channel_id=100, sticky_messages=False)
    await dispatcher._bundle_store.save('k', bundle.to_dict())  # pylint: disable=protected-access

    await dispatcher._remove_mutable('k')  # pylint: disable=protected-access

    release_spy.assert_awaited_with('k')
    assert await dispatcher._bundle_store.load('k') is None  # pylint: disable=protected-access
