...
# ---------------------------------------------------------------------------
# bundle_lock contention / timeout
# ---------------------------------------------------------------------------

@pytest.mark.asyncio
async def test_bundle_lock_times_out_and_proceeds_when_contended(monkeypatch):
    '''When another holder owns the lock, bundle_lock polls until the wait
    deadline then proceeds without it — a torn write beats dropping the op —
    and never deletes the foreign holder's lock on exit.'''
    monkeypatch.setattr(broker_registry, 'BUNDLE_LOCK_WAIT_SECONDS', 0.05)
    monkeypatch.setattr(broker_registry, 'BUNDLE_LOCK_POLL_INTERVAL_SECONDS', 0.01)
    client = fakeredis.aioredis.FakeRedis(decode_responses=True)
    reg = RedisBrokerRegistry(RedisManager.from_client(client))
    lock_key = f'{BUNDLE_LOCK_KEY_PREFIX}b1'
    await client.set(lock_key, 'someone-else')  # pre-occupy so SET NX fails
    entered = False
    async with reg.bundle_lock('b1'):
        entered = True
