...
from discord_seam_broker.utils.integrations.s3 import upload_file, get_file, delete_file, ObjectStorageException

@pytest.fixture
def mock_s3_client():
    with patch("discord_seam_broker.utils.integrations.s3.client") as mock_client_constructor:
...
def test_get_file_matching_etag_no_warning(mock_s3_client, tmp_path, mocker): #pylint:disable=redefined-outer-name
    ...
    mock_logger = mocker.patch('discord_seam_broker.utils.integrations.s3.logger')
    fake_data = b'downloaded content'
