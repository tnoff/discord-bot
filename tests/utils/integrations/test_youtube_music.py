...
from discord_bot.services.search.utils.integrations.youtube_music import YoutubeMusicClient, YoutubeMusicRetryException
...
def test_youtube_music_client(mocker):
    mocker.patch('discord_bot.services.search.utils.integrations._youtube_music_impl.YTMusic', side_effect=yield_youtube_mock([]))
...
def test_youtube_music_client_with_data(mocker):
    ...
    mocker.patch('discord_bot.services.search.utils.integrations._youtube_music_impl.YTMusic', side_effect=yield_youtube_mock(results))
    x = YoutubeMusicClient()
...
def test_youtube_music_server_error_429(mocker):
    ...
    mocker.patch('discord_bot.services.search.utils.integrations._youtube_music_impl.YTMusic', side_effect=_make_error_mock(error))
