...
def test_youtube_playlist_get(mocker):
    mocker.patch('discord_bot.services.search.utils.integrations.youtube.build', side_effect=google_api_build)
    x = YoutubeClient('foo')
...
def test_youtube_playlist_get_with_page_token(mocker):
    mocker.patch('discord_bot.services.search.utils.integrations.youtube.build', side_effect=google_api_build_with_page)
    x = YoutubeClient('foo')
    result = x.playlist_get('1234')
    assert len(result.items) == 2
    assert '1234ABC' in result.items[0].search_string
