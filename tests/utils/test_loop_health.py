...
from discord_core.utils.loop_health import (DEFAULT_STALE_AFTER_SECONDS, health_aware_queue_get,
...
def test_registry_mark_stopped_excludes_named_loops_and_ignores_the_rest():
    # cog_unload / stop name the loops they cancelled; a draining pod must not
    # fail its own liveness probe, and a name that never registered here (a loop
    # this deployment mode doesn't run) must be silently ignored, not raise.
    clock = FakeClock()
    registry = LoopHealthRegistry()
    stopped = registry.register('draining', time_func=clock)
    running = registry.register('busy', time_func=clock)
    registry.mark_stopped('draining', 'never-existed')  # no KeyError on the stranger
    assert stopped.status == LoopStatus.STOPPED
    assert running.status == LoopStatus.OK
    assert registry.is_healthy
