...
from discord_core.utils.otel import (async_otel_span_wrapper, create_observable_gauge,
...
class MessageDispatcher(DispatchClientBase):
    ...
    async def _remove_mutable(self, key: str):
        ...
        Acquires the same per-key execution lock as :meth:`_process_mutable` so
        a remove cannot run concurrently with an in-flight create/update for the
        same bundle. Without the lock a remove issued during the Discord round-trip
        of a create (e.g. the search→enqueue status-message handoff) loads the
        bundle before the create has persisted it, deletes nothing, and the
        message the create then sends is orphaned on screen — surfacing as a
        duplicate status message. On contention we re-enqueue so the remove runs
        after the create finishes and has saved the message_id.
        '''
        acquired = await self._work_queue.acquire_lock(key)
        if not acquired:
