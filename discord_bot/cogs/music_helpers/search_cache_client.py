...
class SearchCacheClient():
    ...
    def iterate(self, source_download: SourceDownload) -> bool:
        ...
        existing = self.db_session.query(SearchString).\
            filter(SearchString.search_string == source_download.source_dict.search_string).first()
        if existing:
            ...
            return True
        search = SearchString(
            search_string=source_download.source_dict.search_string,
    ...
    def check_cache(self, source_dict: SourceDict) -> str:
        ...
        if source_dict.search_type not in VALID_CACHE_TYPES:
            return None
        existing = self.db_session.query(SearchString).\
            filter(SearchString.search_string == source_dict.search_string).first()
        if existing:
