...
class MessageQueue():
    ...
    def update_multiple_mutable(self, index_name: str, text_channel: TextChannel,
                                sticky_messages: bool = True) -> bool:
        '''
        ...
        sticky_messages: Set sticky messages so they only appear at the bottom
        '''
        ...
        if index_name not in self.mutable_bundles:
            if text_channel is None:
                raise MessageQueueException(f'Cannot create new message bundle "{index_name}" without a valid text_channel')
    ...
    async def update_mutable_bundle_content(self, index_name: str, message_content: List[str],
        ...
        delete_after: Set delete after on message dispatch
        '''
        ...
        dispatch_functions = bundle.get_message_dispatch(message_content, clear_existing=should_clear, delete_after=delete_after)
