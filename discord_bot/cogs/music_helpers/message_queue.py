...
from discord_bot.cogs.music_helpers.batched_message import BatchedMessageItem, ItemStatus
...
class MessageQueue():
    ...
    def add_items_to_batch(self, guild_id: int, source_dicts: List[SourceDict], send_function: Callable) -> str:
        '''
        ...
        if guild_id not in self.pending_batches:
            self.pending_batches[guild_id] = BatchedMessageItem(guild_id, self.batch_size)
        ...
        for source_dict in source_dicts:
            if not batch.add_source_dict(source_dict):
                ...
                batch = BatchedMessageItem(guild_id, self.batch_size)
    ...
    def update_batch_item(self, batch_id: str, source_uuid: str, status: ItemStatus, error_msg: str = None) -> bool:
        '''
