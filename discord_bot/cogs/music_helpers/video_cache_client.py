...
class VideoCacheClient():
    ...
    def __ensure_guild(self, db_session: Session, guild_id: str):
        ...
        '''
        guild = retry_database_commands(db_session, partial(database_functions.get_guild, db_session, str(guild_id)))
        if guild:
            return guild
        guild = Guild(
            server_id=str(guild_id),
        )
        retry_database_commands(db_session, partial(database_functions.add_guild, db_session, guild))
        return guild
    ...
    def __ensure_guild_video(self, db_session: Session, guild: Guild, video_cache: VideoCache):
        ...
        '''
        video_guild_cache = retry_database_commands(db_session, partial(database_functions.get_video_cache_guild, db_session, video_cache.id, guild.id))
        ...
        retry_database_commands(db_session, partial(database_functions.add_video_cache_guild, db_session, video_guild_cache))
        return video_guild_cache
    ...
    def verify_cache(self):
        ...
        with otel_span_wrapper(f'{OTEL_SPAN_PREFIX}.verify_cache', kind=SpanKind.INTERNAL):
            ...
            with self.session_generator() as db_session:
                for item in retry_database_commands(db_session, partial(database_functions.get_all_video_cache, db_session)):
    ...
    def iterate_file(self, media_download: MediaDownload) -> bool:
        ...
        with otel_span_wrapper(f'{OTEL_SPAN_PREFIX}.iterate_file', kind=SpanKind.INTERNAL, attributes=attributes):
            ...
            with self.session_generator() as db_session:
                video_cache = retry_database_commands(db_session, partial(database_functions.get_video_cache, db_session, media_download.webpage_url))
                if video_cache:
                    video_cache.count += 1
                    video_cache.last_iterated_at = now
                    # Unmark deletion here just in case
                    video_cache.ready_for_deletion = False
                    self.__ensure_guild_video(db_session, self.__ensure_guild(db_session, media_download.media_request.guild_id), video_cache)
                ...
                retry_database_commands(db_session, partial(database_functions.add_video_cache, db_session, cache_item))
                self.__ensure_guild_video(db_session, self.__ensure_guild(db_session, media_download.media_request.guild_id), cache_item)
                return True
    ...
    def get_webpage_url_item(self, media_request: MediaRequest) -> MediaDownload:
        ...
        with otel_span_wrapper(f'{OTEL_SPAN_PREFIX}.get_webpage_url', kind=SpanKind.INTERNAL, attributes=attributes):
            with self.session_generator() as db_session:
                video_cache = retry_database_commands(db_session, partial(database_functions.get_video_cache, db_session, media_request.search_string))
                if not video_cache:
                    return None
    ...
    def search_existing_file(self, extractor: str, video_id: str) -> VideoCache:
        ...
        with otel_span_wrapper(f'{OTEL_SPAN_PREFIX}.search_existing', kind=SpanKind.INTERNAL, attributes=attributes):
            with self.session_generator() as db_session:
                existing = retry_database_commands(db_session, partial(database_functions.query_existing_files, db_session, extractor, video_id))
                if existing:
                    return existing
                return None
        ...
        with otel_span_wrapper(f'{OTEL_SPAN_PREFIX}.remove_video', kind=SpanKind.INTERNAL):
            with self.session_generator() as db_session:
                for video_cache_id in video_cache_ids:
                    ...
                    backup_item = retry_database_commands(db_session, partial(database_functions.check_video_backup_exists, db_session, video_cache_id))
                    if backup_item:
                        delete_file(backup_item.bucket_name, backup_item.object_path)
                        retry_database_commands(db_session, partial(database_functions.delete_backup_item, db_session, backup_item.id))
                    retry_database_commands(db_session, partial(database_functions.remove_video_cache, db_session, video_cache))
            return True
    ...
    def ready_remove(self):
        ...
        with otel_span_wrapper(f'{OTEL_SPAN_PREFIX}.ready_remove', kind=SpanKind.INTERNAL):
            with self.session_generator() as db_session:
                cache_count = retry_database_commands(db_session, partial(database_functions.video_cache_count, db_session))
    ...
    def object_storage_backup(self, video_cache_id: int) -> bool:
        ...
        with otel_span_wrapper(f'{OTEL_SPAN_PREFIX}.object_storage_backup', kind=SpanKind.INTERNAL):
            if self.storage_option not in [el.value for el in StorageOptions]:
                return False
            with self.session_generator() as db_session:
                item_exists = retry_database_commands(db_session, partial(database_functions.check_video_backup_exists, db_session, video_cache_id))
                if item_exists:
                    return True
                ...
                retry_database_commands(db_session, partial(database_functions.create_video_cache_backup, db_session, video_cache_id, 's3', self.bucket_name, str(video_cache_item.base_path)))
                return True
