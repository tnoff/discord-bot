...
class DownloadClient():
    ...
    def clear_guild_queue(self, guild_id: int,
        ...
        if not self._direct_input_queue.queues:
    ...
    def get_input_nowait(self) -> MediaRequest:
        ...
        try:
            return self._direct_input_queue.get_nowait()
        except QueueEmpty:
    ...
    async def run(self, shutdown_event: asyncio.Event) -> None:
        ...
        DIRECT items always run without waiting: they are checked first before
        any backoff.  If a DIRECT item arrives mid-wait, backoff_wait raises
        DirectItemAvailableException to interrupt the wait early.
        '''
        await sleep(0.01)
        try:
            media_request = self._direct_input_queue.get_nowait()
            self._direct_available.clear()
        except QueueEmpty:
            try:
