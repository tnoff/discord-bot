...
class MusicPlayer:
    ...
    async def clear_queue_messages(self):
        ...
        for queue_message in self.queue_messages:
            await async_retry_discord_message_command(queue_message.delete)
        self.queue_messages = []
    ...
    async def move_queue_message_channel(self, new_channel):
        ...
        for message in self.queue_messages:
            new_messages.append(await async_retry_discord_message_command(new_channel.send, message.content))
        for queue_message in self.queue_messages:
            try:
                await async_retry_discord_message_command(queue_message.delete)
            except NotFound:
    ...
    async def update_queue_strings(self):
        ...
        for (count, queue_message) in enumerate(self.queue_messages):
            ...
            if queue_message.content == new_queue_strings[count]:
                continue
            await async_retry_discord_message_command(queue_message.edit, content=new_queue_strings[count])
        if len(self.queue_messages) < len(new_queue_strings):
            for table in new_queue_strings[-(len(new_queue_strings) - len(self.queue_messages)):]:
                self.queue_messages.append(await async_retry_discord_message_command(self.text_channel.send, table))
        await self.release_lock()
    ...
    async def __player_loop(self):
        ...
        if not source['file_path'].exists():
            await async_retry_discord_message_command(self.text_channel.send, f'Unable to play "{source["title"]}", local file dissapeared')
            return
...
class Music(CogHelper): #pylint:disable=too-many-public-methods
    ...
    def __init__(self, bot, logger, settings, db_engine):
        ...
        self.server_playlist_max = self.settings['music'].get('server_playlist_max', SERVER_PLAYLIST_MAX_DEFAULT)
        self.max_song_length = self.settings['music'].get('max_song_length', MAX_SONG_LENGTH_DEFAULT)
        self.disconnect_timeout = self.settings['music'].get('disconnect_timeout', DISCONNECT_TIMEOUT_DEFAULT)
        self.download_dir = self.settings['music'].get('download_dir', None)
        self.enable_audio_processing = self.settings['music'].get('enable_audio_processing', False)
        self.enable_cache = self.settings['music'].get('enable_cache_files', False)
        self.max_cache_files = self.settings['music'].get('max_cache_files', MAX_CACHE_FILES_DEFAULT)
        self.banned_videos_list = self.settings['music'].get('banned_videos_list', [])
        spotify_client_id = self.settings['music'].get('spotify_client_id', None)
        spotify_client_secret = self.settings['music'].get('spotify_client_secret', None)
        youtube_api_key = self.settings['music'].get('youtube_api_key', None)
    ...
    async def __download_files(self):
        ...
        try:
            ...
            await async_retry_discord_message_command(source_dict['message'].delete)
            return
        ...
        if player.play_queue.full():
            ...
            await async_retry_discord_message_command(source_dict['message'].edit,
        ...
        if source_download is None:
            await async_retry_discord_message_command(source_dict['message'].edit, content=f'Issue downloading video "{source_dict["search_string"]}", skipping',
                                                delete_after=player.delete_after)
    ...
    async def __check_database_session(self, ctx):
        ...
        if not self.db_session:
            await async_retry_discord_message_command(ctx.send, 'Functionality not available, database is not enabled')
            return False
        return True
    ...
    async def __check_author_voice_chat(self, ctx, check_voice_chats=True):
        ...
        try:
            ...
            await async_retry_discord_message_command(ctx.send, f'"{ctx.author.name}" not in voice chat channel. Please join one and try again',
                                                delete_after=self.delete_after)
            return None
        ...
        if channel.guild.id is not ctx.guild.id:
            await async_retry_discord_message_command(ctx.send, 'User not joined to channel bot is in, ignoring command',
                                     delete_after=self.delete_after)
            return False
        return channel
    ...
    async def connect_(self, ctx):
        ...
        if vc:
            ...
            try:
                ...
                return await async_retry_discord_message_command(ctx.send, f'Moving to channel: <{channel}> timed out.')
        else:
            try:
                await channel.connect()
            except asyncio_timeout:
                self.logger.warning(f'Music :: Connecting to channel {channel.id} timed out')
                return await async_retry_discord_message_command(ctx.send, f'Connecting to channel: <{channel}> timed out.')

        await async_retry_discord_message_command(ctx.send, f'Connected to: {channel}', delete_after=self.delete_after)
    ...
    async def play_(self, ctx, *, search: str):
        ...
        if player.play_queue.full():
            return await async_retry_discord_message_command(ctx.send, 'Queue is full, cannot add more songs',
                                                       delete_after=self.delete_after)

        entries = await self.download_client.check_source(search, ctx.guild.id, ctx.author.name, self.bot.loop)
        for entry in entries:
            try:
                message = await async_retry_discord_message_command(ctx.send, f'Downloading and processing "{entry["search_string"]}"')
                ...
                await async_retry_discord_message_command(message.edit, content=f'Unable to add "{search}" to queue, download queue is full', delete_after=self.delete_after)
                return
    ...
    async def skip_(self, ctx):
        ...
        if not vc or not vc.is_connected():
            return await async_retry_discord_message_command(ctx.send, 'I am not currently playing anything',
                                                       delete_after=self.delete_after)
        ...
        vc.stop()
        await async_retry_discord_message_command(ctx.send, 'Skipping song',
                                            delete_after=self.delete_after)
    ...
    async def clear(self, ctx):
        ...
        if player.play_queue.empty():
            return await async_retry_discord_message_command(ctx.send, 'There are currently no more queued songs.',
                                                      delete_after=self.delete_after)
        ...
        for _ in range(player.play_queue.size()):
            ...
            item.delete()
        await player.update_queue_strings()
        return await async_retry_discord_message_command(ctx.send, 'Cleared player queue', delete_after=self.delete_after)
    ...
    async def history_(self, ctx):
        ...
        if player.history.empty():
            return await async_retry_discord_message_command(ctx.send, 'There have been no songs played.',
                                                       delete_after=self.delete_after)
        ...
        for mess in messages:
            await async_retry_discord_message_command(ctx.send, mess, delete_after=self.delete_after)
    ...
    async def shuffle_(self, ctx):
        ...
        if player.play_queue.shutdown:
            return await async_retry_discord_message_command(ctx.send, 'Unable to shuffle queue, player in shutdown',
                                                       delete_after=self.delete_after)
    ...
    async def remove_item(self, ctx, queue_index):
        ...
        if not vc or not vc.is_connected():
            return await async_retry_discord_message_command(ctx.send, 'I am not currently connected to voice',
                                            delete_after=self.delete_after)
        ...
        # Check if player is in shutdown, assume we're shutting down or clearing queue
        if player.play_queue.shutdown:
            return await async_retry_discord_message_command(ctx.send, 'Unable to remove item, player in shutdown',
                                                       delete_after=self.delete_after)

        try:
            queue_index = int(queue_index)
        except ValueError:
            return await async_retry_discord_message_command(ctx.send, f'Invalid queue index {queue_index}',
                                            delete_after=self.delete_after)

        item = player.play_queue.remove_item(queue_index)
        if item is None:
            return async_retry_discord_message_command(ctx.send, f'Unable to remove queue index {queue_index}',
                            delete_after=self.delete_after)
        await async_retry_discord_message_command(ctx.send, f'Removed item {item["title"]} from queue',
                                 delete_after=self.delete_after)
        item.delete()
        await player.update_queue_strings()
    ...
    async def bump_item(self, ctx, queue_index):
        ...
        # Check if player is in shutdown, assume we're shutting down or clearing queue
        if player.play_queue.shutdown:
            return await async_retry_discord_message_command(ctx.send, 'Unable to bump item, player in shutdown',
                                                       delete_after=self.delete_after)
        try:
            queue_index = int(queue_index)
        except ValueError:
        ...
        if item is None:
            return await async_retry_discord_message_command(ctx.send, f'Unable to bump queue index {queue_index}',
                                            delete_after=self.delete_after)
        await async_retry_discord_message_command(ctx.send, f'Bumped item {item["title"]} to top of queue',
                                 delete_after=self.delete_after)
    ...
    async def move_messages_here(self, ctx):
        ...
        if ctx.channel.id == player.text_channel.id:
            return await async_retry_discord_message_command(ctx.send, f'I am already sending messages to channel {ctx.channel.name}',
                                                       delete_after=self.delete_after)
    ...
    async def __get_playlist(self, playlist_index, ctx):
        try:
            ...
            await async_retry_discord_message_command(ctx.send, f'Invalid playlist index {playlist_index}', delete_after=self.delete_after)
            return None
        ...
        if not playlist_items:
            await async_retry_discord_message_command(ctx.send, 'No playlists in database',
                                                delete_after=self.delete_after)
            return None
        try:
    ...
    async def playlist(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            await async_retry_discord_message_command(ctx.send, 'Invalid sub command passed...', delete_after=self.delete_after)
    ...
    async def __playlist_create(self, ctx, name):
        ...
        if not await self.__check_database_session(ctx):
            async_retry_discord_message_command(ctx.send, 'Database not set, cannot use playlist functions', delete_after=self.delete_after)
            return None
        ...
        if PLAYHISTORY_PREFIX in playlist_name.lower():
            await async_retry_discord_message_command(ctx.send, f'Unable to create playlist "{name}", name cannot contain {PLAYHISTORY_PREFIX}')
            return None
        ...
        if server_playlist_count >= self.server_playlist_max:
            await async_retry_discord_message_command(ctx.send, f'Unable to create playlist "{name}", already hit max playlists for server')
            return None
        ...
        try:
            ...
            self.db_session.commit()
            await async_retry_discord_message_command(ctx.send, f'Unable to create playlist "{name}", name likely already exists')
            return None
        self.logger.info(f'Music :: Playlist created "{playlist.name}" with ID {playlist.id} in guild {ctx.guild.id}')
        await async_retry_discord_message_command(ctx.send, f'Created playlist "{name}"',
                                            delete_after=self.delete_after)
        return playlist
    ...
    async def playlist_list(self, ctx):
        ...
        if not await self.__check_author_voice_chat(ctx):
            return
        if not await self.__check_database_session(ctx):
            return async_retry_discord_message_command(ctx.send, 'Database not set, cannot use playlist functions', delete_after=self.delete_after)
        playlist_items = self.db_session.query(Playlist).\
            filter(Playlist.server_id == str(ctx.guild.id)).order_by(Playlist.created_at.asc())
        playlist_items = [p for p in playlist_items if PLAYHISTORY_PREFIX not in p.name]

        if not playlist_items:
            return await async_retry_discord_message_command(ctx.send, 'No playlists in database',
                                            delete_after=self.delete_after)
    ...
    async def __playlist_item_add(self, ctx, playlist_index, search):
        ...
        for entry in source_entries:
            ...
            if source is None:
                await async_retry_discord_message_command(ctx.send, f'Unable to find video for search {search}')
                continue
            ...
            try:
                ...
                async_retry_discord_message_command(ctx.send, f'Cannot add more items to playlist "{playlist.name}", already max size', delete_after=self.delete_after)
                return
            if playlist_item:
                await async_retry_discord_message_command(ctx.send, f'Added item "{source["title"]}" to playlist {playlist_index}', delete_after=self.delete_after)
                continue
            await async_retry_discord_message_command(ctx.send, 'Unable to add playlist item, likely already exists', delete_after=self.delete_after)
    ...
    async def playlist_item_search(self, ctx, playlist_index, *, search: str):
        ...
        '''
        if not await self.check_user_role(ctx):
            return await async_retry_discord_message_command(ctx.send, 'Unable to verify user role, ignoring command',
                                            delete_after=self.delete_after)
        if not await self.__check_author_voice_chat(ctx):
            return
        if not await self.__check_database_session(ctx):
            return async_retry_discord_message_command(ctx.send, 'Database not set, cannot use playlist functions',
                                      delete_after=self.delete_after)
        ...
        if not items:
            return await async_retry_discord_message_command(ctx.send, f'No playlist items in matching string "{search}"',
                                            delete_after=self.delete_after)
    ...
    async def playlist_item_remove(self, ctx, playlist_index, song_index):
        ...
        try:
            ...
            return await async_retry_discord_message_command(ctx.send, f'Invalid item index {song_index}',
                                            delete_after=self.delete_after)
        if song_index < 1:
            return await async_retry_discord_message_command(ctx.send, f'Invalid item index {song_index}',
                                            delete_after=self.delete_after)
        ...
        try:
            ...
            self.db_session.commit()
            return await async_retry_discord_message_command(ctx.send, f'Removed item {song_index} from playlist',
                                            delete_after=self.delete_after)
        except IndexError:
            return await async_retry_discord_message_command(ctx.send, f'Unable to find item {song_index}',
                                            delete_after=self.delete_after)
    ...
    async def __playlist_delete(self, ctx, playlist_index):
        ...
        self.db_session.commit()
        return await async_retry_discord_message_command(ctx.send, f'Deleted playlist {playlist_index}',
                                                   delete_after=self.delete_after)
    ...
    async def playlist_rename(self, ctx, playlist_index, *, playlist_name: str):
        ...
        self.db_session.commit()
        return await async_retry_discord_message_command(ctx.send, f'Renamed playlist {playlist_index} to name "{playlist_name}"', delete_after=self.delete_after)
    ...
    async def __playlist_queue_save(self, ctx, name, is_history=False):
        ...
        try:
            ...
            return await async_retry_discord_message_command(ctx.send, 'No player connected, no queue to save',
                                                       delete_after=self.delete_after)
        ...
        if len(queue_copy) == 0:
            return await async_retry_discord_message_command(ctx.send, 'There are no songs to add to playlist',
                                                       delete_after=self.delete_after)
        ...
        for data in queue_copy:
            try:
                ...
                break
            if playlist_item:
                await async_retry_discord_message_command(ctx.send, f'Added item "{data["title"]}" to playlist', delete_after=self.delete_after)
                continue
            await async_retry_discord_message_command(ctx.send, f'Unable to add playlist item "{data["title"]}", likely already exists', delete_after=self.delete_after)
        await async_retry_discord_message_command(ctx.send, f'Finished adding items to playlist "{name}"', delete_after=self.delete_after)
        if is_history:
            player.history.clear()
            await async_retry_discord_message_command(ctx.send, 'Cleared history', delete_after=self.delete_after)
        return
    ...
    async def playlist_random_play(self, ctx, sub_command: Optional[str] = ''):
        ...
        if sub_command:
            try:
                ...
                async_retry_discord_message_command(ctx.send, f'Using default number of max songs {DEFAULT_RANDOM_QUEUE_LENGTH}', delete_after=self.delete_after)
        history_playlist = self.db_session.query(Playlist).\
            filter(Playlist.server_id == str(ctx.guild.id)).\
            filter(Playlist.is_history == True).first()

        if not history_playlist:
            return await async_retry_discord_message_command(ctx.send, 'Unable to find history for server', delete_after=self.delete_after)
    ...
    async def __delete_non_existing_item(self, item, ctx):
        ...
        await async_retry_discord_message_command(ctx.send, content=f'Unable to find video "{item.video_id}" in playlist, deleting',
                                            delete_after=self.delete_after)
        self.db_session.delete(item)
        self.db_session.commit()
    ...
    async def __playlist_queue(self, ctx, playlist, shuffle, max_num, is_history=False):
        ...
        if shuffle:
            await async_retry_discord_message_command(ctx.send, 'Shuffling playlist items',
                                                delete_after=self.delete_after)
            for _ in range(NUM_SHUFFLES):
        ...
        if max_num:
            if max_num < 0:
                await async_retry_discord_message_command(ctx.send, f'Invalid number of songs {max_num}',
                                                    delete_after=self.delete_after)
                return
        ...
        for item in playlist_items:
            message = await async_retry_discord_message_command(ctx.send, f'Downloading and processing "{item.title}"')
            try:
                ...
                await async_retry_discord_message_command(message.edit, content=f'Unable to add item "{item.title}" with id "{item.video_id}" to queue, queue is full',
                                                    delete_after=self.delete_after)
                broke_early = True
                break
            except PutsBlocked:
        ...
        if broke_early:
            await async_retry_discord_message_command(ctx.send, f'Added as many songs in playlist "{playlist_name}" to queue as possible, but hit limit',
                                                delete_after=self.delete_after)
        elif max_num:
            await async_retry_discord_message_command(ctx.send, f'Added {max_num} songs from "{playlist_name}" to queue',
                                                delete_after=self.delete_after)
        else:
            await async_retry_discord_message_command(ctx.send, f'Added all songs in playlist "{playlist_name}" to queue',
                                                delete_after=self.delete_after)
    ...
    async def playlist_merge(self, ctx, playlist_index_one, playlist_index_two):
        ...
        if not playlist_one:
            return async_retry_discord_message_command(ctx.send, f'Cannot find playlist {playlist_index_one}', delete_after=self.delete_after)
        if not playlist_two:
            return async_retry_discord_message_command(ctx.send, f'Cannot find playlist {playlist_index_two}', delete_after=self.delete_after)
        ...
        for item in query:
            try:
                ...
                async_retry_discord_message_command(ctx.send, f'Cannot add more items to playlist "{playlist_one.name}", already max size', delete_after=self.delete_after)
                return
            if playlist_item:
                await async_retry_discord_message_command(ctx.send, f'Added item "{item.title}" to playlist {playlist_index_one}', delete_after=self.delete_after)
                continue
            await async_retry_discord_message_command(ctx.send, f'Unable to add playlist item "{item.title}", likely already exists', delete_after=self.delete_after)
