...
from discord_bot.utils.instrument import otel_span_wrapper, command_wrapper, AttributeNaming, HeartbeatMetricNaming
...
SEND_MESSAGE_HEARTBEAT_GAUGE = METER_PROVIDER.create_gauge(HeartbeatMetricNaming.MUSIC_SEND_MESSAGES.value, unit='number', description='Send messages heartbeat')
CLEANUP_PLAYERS_HEARTBEAT_GAUGE = METER_PROVIDER.create_gauge(HeartbeatMetricNaming.MUSIC_CLEANUP_PLAYERS.value, unit='number', description='Cleanup players heartbeat')
CACHE_CLEANUP_HEARTBEAT_GAUGE = METER_PROVIDER.create_gauge(HeartbeatMetricNaming.MUSIC_CACHE_CLEANUP.value, unit='number', description='Cache cleanup heartbeat')
DOWNLOAD_FILES_HEARTBEAT_GAUGE = METER_PROVIDER.create_gauge(HeartbeatMetricNaming.MUSIC_DOWNLOAD_FILES.value, unit='number', description='Download files heartbeat')
HISTORY_PLAYLIST_HEARTBEAT_GAUGE = METER_PROVIDER.create_gauge(HeartbeatMetricNaming.MUSIC_PLAYLIST_HISTORY.value, unit='number', description='Playlist history heartbeat')
...
class Music(CogHelper): #pylint:disable=too-many-public-methods
    ...
    async def playlist_history_update(self):
        ...
        if self.check_wait_playlist_history():
            HISTORY_PLAYLIST_HEARTBEAT_GAUGE.set(1)
            self.update_playlist_history_timestamp()
        try:
    ...
    async def send_messages(self):
        ...
        if self.check_wait_send_message():
            SEND_MESSAGE_HEARTBEAT_GAUGE.set(1)
            self.update_send_message_loop_timestamp()
    ...
    async def cleanup_players(self):
        ...
        await sleep(1)
        CLEANUP_PLAYERS_HEARTBEAT_GAUGE.set(1)
    ...
    async def cache_cleanup(self):
        ...
        await sleep(1)
        CACHE_CLEANUP_HEARTBEAT_GAUGE.set(1)
    ...
    async def youtube_backoff_time(self, minimum_wait_time: int, max_variance: int):
        ...
        while True:
            ...
            DOWNLOAD_FILES_HEARTBEAT_GAUGE.set(1)
