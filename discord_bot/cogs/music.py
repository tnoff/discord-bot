...
from discord_bot.cogs.music_helpers.common import SearchType, MessageType, MultipleMutableType, MediaRequestLifecycleStage, PLAYHISTORY_PREFIX
...
from discord_bot.database import PlaylistItem
...
class Music(CogHelper): #pylint:disable=too-many-public-methods
    ...
    def __cache_count_callback(self, _options):
        ...
        with self.with_db_session() as db_session:
            cache_file_count = retry_database_commands(db_session, partial(database_functions.video_cache_count, db_session))
            return [
    ...
    async def playlist_history_update(self):
        ...
        with otel_span_wrapper(f'{OTEL_SPAN_PREFIX}.playlist_history_update', kind=SpanKind.CONSUMER):
            with self.with_db_session() as db_session:
                ...
                retry_database_commands(db_session, partial(database_functions.delete_existing_item, db_session, history_item.media_download.webpage_url, history_item.playlist_id))
                ...
                if delta > 0:
                    ...
                    retry_database_commands(db_session, partial(database_functions.delete_extra_items, db_session, history_item.playlist_id, delta))
    ...
    async def cache_cleanup(self):
        ...
        with otel_span_wrapper(f'{OTEL_SPAN_PREFIX}.cache_cleanup', kind=SpanKind.CONSUMER):
            ...
            with self.with_db_session() as db_session:
                ...
                for video_cache in retry_database_commands(db_session, partial(database_functions.list_ready_cache_files, db_session)):
                ...
                for video_cache in retry_database_commands(db_session, partial(database_functions.list_non_backup_files, db_session)):
    ...
    def __get_history_playlist(self, guild_id: int):
        ...
        if not self.db_engine:
            return None
        with self.with_db_session() as db_session:
            history_playlist = retry_database_commands(db_session, partial(database_functions.find_history_playlist, db_session, guild_id))
            if history_playlist:
                return history_playlist.id
            history_playlist = retry_database_commands(db_session, partial(database_functions.create_history_playlist, db_session, guild_id))
            return history_playlist.id
    ...
    async def __get_playlist_public_view(self, playlist_id: int, guild_id: str):
        ...
        with self.with_db_session() as db_session:
            ...
            for (count, playlist_obj) in enumerate(retry_database_commands(db_session, partial(database_functions.get_non_history_playlists, db_session, guild_id))):
    ...
    async def __get_playlist(self, playlist_index: int, ctx: Context):
        ...
        with self.with_db_session() as db_session:
            if index > 0:
                if not retry_database_commands(db_session, partial(database_functions.check_playlist_count, db_session, str(ctx.guild.id))):
                    message_context = MessageContext(ctx.guild.id, ctx.channel.id)
            ...
            is_history = False
            if index == 0:
                playlist = retry_database_commands(db_session, partial(database_functions.find_history_playlist, db_session, str(ctx.guild.id)))
                is_history = True
            else:
                playlist = retry_database_commands(db_session, partial(database_functions.list_non_history_playlists, db_session, str(ctx.guild.id), (index - 1)))[0]
            if not playlist:
                message_context = MessageContext(ctx.guild.id, ctx.channel.id)
                message_context.function = partial(ctx.send, f'Invalid playlist index {playlist_index}', delete_after=self.delete_after)
                self.message_queue.send_single_immutable([message_context])
    ...
    async def __playlist_create(self, ctx: Context, name: str):
        ...
        if PLAYHISTORY_PREFIX in playlist_name.lower():
            ...
            return None
        with self.with_db_session() as db_session:
            existing_playlist = retry_database_commands(db_session, partial(database_functions.check_for_playlist, db_session, playlist_name, str(ctx.guild.id)))
            ...
            playlist = retry_database_commands(db_session, partial(database_functions.create_playlist, db_session, playlist_name, str(ctx.guild.id)))
    ...
    async def playlist_list(self, ctx: Context):
        ...
        if not await self.__check_database_session(ctx):
            return
        with self.with_db_session() as db_session:
            history_playlist = retry_database_commands(db_session, partial(database_functions.find_history_playlist, db_session, str(ctx.guild.id)))
            playlist_items = retry_database_commands(db_session, partial(database_functions.get_non_history_playlists, db_session, str(ctx.guild.id)))
    ...
    def __playlist_insert_item(self, playlist_id: int, video_url: str, video_title: str, video_uploader: str):
        ...
        with self.with_db_session() as db_session:
            ...
            existing_item = retry_database_commands(db_session, partial(database_functions.check_existing_item, db_session, playlist_id, video_url))
            if existing_item:
                return None

            playlist_item_id = retry_database_commands(db_session, partial(database_functions.create_new_item, db_session, video_title,
    ...
    async def playlist_item_remove(self, ctx: Context, playlist_index: int, video_index: int):
        ...
        with self.with_db_session() as db_session:
            item = retry_database_commands(db_session, partial(database_functions.remove_playlist_item_remove, db_session, playlist_id, (video_index - 1)))
    ...
    async def playlist_show(self, ctx: Context, playlist_index: int):
        ...
        with self.with_db_session() as db_session:
            ...
            table = DapperTable(header_options=DapperTableHeaderOptions(headers), rows_per_message=15)
            total = 0
            for (count, item) in enumerate(retry_database_commands(db_session, partial(database_functions.get_playlist_items, db_session, playlist_id))): #pylint:disable=protected-access
                uploader = item.uploader or ''
                table.add_row([
                    f'{count + 1}',
                    f'{item.title} /// {uploader}',
    ...
    async def __playlist_queue(self, ctx: Context, player: MusicPlayer, playlist_id: int, shuffle: bool, max_num: int, is_history: bool = False):
        ...
        with self.with_db_session() as db_session:
            ...
            playlist_items = []
            for item in retry_database_commands(db_session, partial(database_functions.get_playlist_items, db_session, playlist_id)):
            ...
            retry_database_commands(db_session, partial(database_functions.playlist_update_queued, db_session, playlist_id))
    ...
    async def playlist_merge(self, ctx: Context, playlist_index_one: str, playlist_index_two: str):
        ...
        if not playlist_two_id:
            ...
            return
        with self.with_db_session() as db_session:
            for item in retry_database_commands(db_session, partial(database_functions.get_playlist_items, db_session, playlist_two_id)):
                try:
