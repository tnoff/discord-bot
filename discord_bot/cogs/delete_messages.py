...
class DeleteMessages(CogHelper):
    ...
    def __init__(self, bot, logger, settings):
        super().__init__(bot, logger, settings, enable_loop=True, settings_prefix='delete_messages', section_schema=DELETE_MESSAGES_SCHEMA)
        if not self.settings['include']['delete_messages']:
