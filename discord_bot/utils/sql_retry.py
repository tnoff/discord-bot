...
from discord_bot.utils.otel import otel_span_wrapper, async_otel_span_wrapper, AttributeNaming
...
async def async_retry_database_commands(db_session, function: Callable, attempts: int = 3, interval: int = 1):
    '''
    Async version of retry_database_commands for use with AsyncSession.

    db_session : Valid AsyncSession being used
    function   : Async callable (no args) to execute
    attempts : Number of attempts
    interval : Sleep interval
    '''
    async with async_otel_span_wrapper(f'{OTEL_SPAN_PREFIX}.retry_db_command', kind=SpanKind.CLIENT) as span:
        count = 0
        while True:
            span.set_attributes({
                AttributeNaming.RETRY_COUNT.value: count,
            ...
            try:
                ...
                if count > attempts:
                    ...
                    raise
                await db_session.rollback()
                await asyncio.sleep(0.5 * (attempts * interval))
                continue
